    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <div>
                ...
                <h1 class="text-xl font-bold text-gray-800">Edit Item</h1>
            </div>
        </div>
    </x-slot>
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
                ...
                <div class="bg-gradient-to-r from-theme-from to-theme-to px-8 py-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h2 class="text-white text-xl font-bold">Item Record Update</h2>
                            <p class="text-sky-100 text-sm mt-0.5">Review and update the item's information below</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="px-8 py-7">
                    ...
                    <form action="{{ route('pharmacy.items.update', $item->id) }}" method="POST" class="space-y-5">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="group">
                            ...
                            <input type="text" id="commercial_name" name="commercial_name"
                                value="{{ old('commercial_name', $item->commercial_name) }}"
                                required autofocus
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" id="scientific_name" name="scientific_name"
                                    value="{{ old('scientific_name', $item->scientific_name) }}"
                                    required
                            ...
                            <div>
                                ...
                                <select id="form" name="form"
                                    ...
                                    @foreach(['tablet' => 'Tablet', 'capsule' => 'Capsule', 'syrup' => 'Syrup', 'cream' => 'Cream', 'ointment' => 'Ointment', 'injection' => 'Injection', 'suspension' => 'Suspension', 'drops' => 'Drops'] as $value => $label)
                                        <option value="{{ $value }}" {{ old('form', $item->form) === $value ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="category" name="category"
                                    ...
                                    @foreach(['medicine' => 'Medicine', 'supplement' => 'Supplement', 'cosmetic' => 'Cosmetic', 'other' => 'Other'] as $value => $label)
                                        <option value="{{ $value }}" {{ old('category', $item->category) === $value ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            ...
                            <input type="text" id="location_in_pharmacy" name="location_in_pharmacy"
                                value="{{ old('location_in_pharmacy', $item->location_in_pharmacy) }}"
