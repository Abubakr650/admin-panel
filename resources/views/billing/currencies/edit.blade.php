    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <h1 class="text-xl font-bold text-gray-800">Edit Currency: {{ $currency->code }}</h1>
            </div>
        </div>
    </x-slot>
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
                <div class="bg-gradient-to-r from-theme-from to-theme-to px-8 py-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h2 class="text-white text-xl font-bold">Edit Currency</h2>
                            <p class="text-sky-100 text-sm mt-0.5">Update details and exchange rate</p>
                        </div>
                    </div>
                </div>
                    ...
                    <form action="{{ route('currencies.update', $currency) }}" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="name" value="{{ old('name', $currency->name) }}" placeholder="e.g. Yemeni Rial" required class="{{ $inputClass('name') }}">
                            ...
                            <div>
                                ...
                                <input type="text" name="code" value="{{ old('code', $currency->code) }}" placeholder="e.g. YER" maxlength="10" required class="{{ $inputClass('code') }} uppercase font-mono tracking-wider">
                        ...
                        <div class="pt-4 border-t border-gray-100">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50/50 p-4 rounded-xl border border-gray-100">
                                <div>
                                    ...
                                    <select name="base_currency_id" class="{{ $inputClass('base_currency_id') }}">
                                        ...
                                        @foreach($currencies as $c)
                                            <option value="{{ $c->id }}" {{ old('base_currency_id', $latestRate?->from_currency_id) == $c->id ? 'selected' : '' }}>{{ $c->code }} - {{ $c->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <input type="number" step="0.000001" name="exchange_rate" value="{{ old('exchange_rate', $latestRate?->rate) }}" placeholder="e.g. 530" class="{{ $inputClass('exchange_rate') }}">
                                    @error('exchange_rate')<p class="text-red-500 text-xs mt-1 font-medium">{{ $message }}</p>@enderror
                                    <p class="text-xs text-gray-400 mt-1.5">Value of 1 Base Currency in this currency.</p>
                                </div>
                            </div>
                        </div>
