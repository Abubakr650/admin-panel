    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <h1 class="text-xl font-bold text-gray-800">Edit Service</h1>
            </div>
        </div>
    </x-slot>
    <div class="p-6">
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
                <div class="bg-gradient-to-r from-theme-from to-theme-to px-8 py-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h2 class="text-white text-xl font-bold">Edit Service</h2>
                            <p class="text-sky-100 text-sm mt-0.5">{{ $service->name }}</p>
                        </div>
                    </div>
                </div>
                <div class="px-8 py-7">
                    ...
                    <form action="{{ route('services.update', $service) }}" method="POST" class="space-y-5">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="col-span-2">
                                ...
                                <input type="text" name="name" value="{{ old('name', $service->name) }}" class="{{ $ic('name') }}">
                                <x-input-error :messages="$errors->get('name')" class="mt-1" />
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-gray-600 mb-1.5">Code</label>
                                <input type="text" name="code" value="{{ old('code', $service->code) }}" class="{{ $ic('code') }}">
                                <x-input-error :messages="$errors->get('code')" class="mt-1" />
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-gray-600 mb-1.5">Default Price ($) <span class="text-red-400">*</span></label>
                                <input type="number" name="default_price" value="{{ old('default_price', $service->default_price) }}" step="0.01" min="0" class="{{ $ic('default_price') }}">
                            ...
                            <div>
                                ...
                                <input type="number" name="duration_minutes" value="{{ old('duration_minutes', $service->duration_minutes) }}" min="0" class="{{ $ic('duration_minutes') }}">
                            </div>
                        ...
                        <div class="border-t border-dashed border-gray-200 pt-2 flex items-center justify-end gap-3">
                            <a href="{{ route('services.show', $service) }}" class="px-5 py-2.5 rounded-xl border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50 transition-all">Cancel</a>
