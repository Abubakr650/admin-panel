    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <div>
                ...
                <h1 class="text-xl font-bold text-gray-800">Edit Doctor</h1>
            </div>
        </div>
    </x-slot>
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
                ...
                <div class="bg-gradient-to-r from-theme-from to-theme-to px-8 py-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h2 class="text-white text-xl font-bold">Doctor Record Update</h2>
                            <p class="text-sky-100 text-sm mt-0.5">Review and update the doctor's information below</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="px-8 py-7">
                    ...
                    <form action="{{ route('doctors.update', $doctor->id) }}" method="POST" class="space-y-5"
                          x-data="{
                            specialty: '{{ old('specialty', $doctor->specialty) }}',
                            degree: '{{ old('degree', $doctor->degree) }}',
                            is_active: {{ old('is_active', $doctor->is_active) ? 'true' : 'false' }},
                            userId: '{{ old('user_id', $doctor->user_id) }}',
                            get userCreateUrl() {
                                let params = new URLSearchParams({
                                    return_to: '{{ route('doctors.edit', $doctor->id) }}',
