<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <div>
                <p class="text-xs text-gray-400 uppercase tracking-widest font-semibold text-theme-from/60">Clinic › Appointments</p>
                <h1 class="text-xl font-bold text-gray-800">Edit Appointment</h1>
            </div>
        </div>
    </x-slot>
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
                ...
                <div class="bg-gradient-to-r from-theme-from to-theme-to px-8 py-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h2 class="text-white text-xl font-bold">Update Appointment</h2>
                            <p class="text-sky-100 text-sm mt-0.5">Modify the appointment details below</p>
                        </div>
                    </div>
                </div>
                    ...
                    <form action="{{ route('appointments.update', $appointment->id) }}" method="POST" class="space-y-5">
                        ...
                        <div>
                            ...
                            <select id="doctor_id" name="doctor_id" required
                                ...
                                style="{{ $selectStyle }}">
                                <option value="" disabled {{ old('doctor_id', $appointment->doctor_id) ? '' : 'selected' }}>Select Doctor</option>
                        ...
                        <div>
                            ...
                            <select id="appointment_status" name="appointment_status" required
                                ...
                                <option value="no_show"   {{ old('appointment_status', $appointment->appointment_status) === 'no_show'   ? 'selected' : '' }}>No Show</option>
                            </select>
                        ...
                        <div>
                            ...
                            <textarea id="appointment_notes" name="appointment_notes" rows="3"
                                ...
                                class="{{ $inputClass('appointment_notes') }} resize-none">{{ old('appointment_notes', $appointment->appointment_notes) }}</textarea>
                        ...
                        <div x-data="{
                                ...
                                selected: '{{ old('parent_appointment_id', $appointment->parent_appointment_id ?? '') }}',
                                loading: false,
                            ...
                            <div class="border-t border-dashed border-gray-200 pt-4">
                                ...
                                @if($appointment->parent)
                                    <div class="mb-2.5 flex items-center gap-3 px-3.5 py-2.5 rounded-xl bg-theme-from/5 border border-theme-from/20 text-xs text-theme-from">
                                        <x-icon name="arrow-up" class="w-4 h-4 text-theme-from/60 shrink-0" />
                                        <span>
                                            Currently linked to:
                                            <a href="{{ route('appointments.show', $appointment->parent->id) }}"
                                               class="font-bold underline hover:text-theme-from/80">
                                                {{ $appointment->parent->appointment_date?->format('M d, Y') }}
